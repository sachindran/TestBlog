...
public class SignUp extends Activity implements OnClickListener{
	...
	private void createAccount(){
		...
		if (cancel) {
			...
			signUp(mUsername.toLowerCase(Locale.getDefault()), mPassword);  //Sachin Email part removed

		}

	}

	private void signUp(final String mUsername , String mPassword)  {//Sachin Email part removed
		Toast.makeText(getApplicationContext(), mUsername , Toast.LENGTH_SHORT).show();     //Sachin Email part removed
		ParseUser user = new ParseUser();
		user.setUsername(mUsername);
		user.setPassword(mPassword);
        //Sachin Email part removed user.setEmail(mEmail);
		 
		user.signUpInBackground(new SignUpCallback() {
		  public void done(ParseException e) {
		    if (e == null) {
		      signUpMsg("Account Created Successfully");
		      Intent in = new Intent(getApplicationContext(),PostListActivity.class);
		      startActivity(in);
		    } else {
